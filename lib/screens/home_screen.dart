...
// Typography system:
// • Google Sans Flex -> expressive/display hierarchy and wallpaper names.
// • Roboto -> body copy, labels, counters and actions.
// The weights intentionally stay around 400–600 so the interface feels light,
// smooth and modern instead of overly bold.
...
// ============================================================
  ...
  void _startFeaturedSlider(int length) {
    ...
    _featuredTimer = Timer.periodic(const Duration(seconds: 5), (_) {
      if (!mounted || _isDraggingFeatured || !_featuredController.hasClients) {
        return;
      }

      final next = (_featuredIndex + 1) % length;
...
// ============================================================
  ...
  void _openPreview(String image, {String category = 'Featured'}) {
    Navigator.of(context).push(
      PageRouteBuilder(
        transitionDuration: const Duration(milliseconds: 380),
        reverseTransitionDuration: const Duration(milliseconds: 280),
        pageBuilder: (context, animation, secondaryAnimation) {
          return PreviewScreen(imageUrl: image, category: category);
        },
...
// loading screen or spinner.
            ...
            final Map<String, dynamic> categories = Map<String, dynamic>.from(
...
// ==================================================
          if (trending.isNotEmpty)
            SliverToBoxAdapter(child: _buildFeatured(trending)),
            ...
            SliverPadding(
              ...
              sliver: SliverList.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final image = trending[realIndex].toString();

                  return Padding(
                    padding: EdgeInsets.only(bottom: 8.h),
                    child: _FeedWallpaperCard(
                      image: image,
                      title: getWallpaperName(image),
                      index: realIndex,
                      height: _feedCardHeight(realIndex),
                      isExpanded: _expandedFeedIndex == realIndex,
                      onToggleExtend: () {
                        HapticFeedback.selectionClick();
                        setState(() {
                          _expandedFeedIndex = _expandedFeedIndex == realIndex
                              ? null
                              : realIndex;
                        });
                      },
                      onTap: () {
                        _openPreview(image, category: 'Trending');
                      },
                    ),
                  );
                },
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          _HeaderButton(icon: Icons.tune_rounded, onTap: _openPreferences),
        ],
      ),
    );
  }
...
// ============================================================
  ...
  Widget _buildIntro() {
    return Padding(
      padding: EdgeInsets.fromLTRB(19.w, 20.h, 19.w, 4.h),
      child: Row(
...
// ============================================================
  ...
  Widget _buildFeatured(List<dynamic> trending) {
    final height = MediaQuery.sizeOf(context).height * .57;

    return Padding(
      padding: EdgeInsets.fromLTRB(12.w, 14.h, 12.w, 0),
      child: SizedBox(
        height: height.clamp(390.h, 540.h),
        child: ClipRRect(
          ...
          child: Stack(
            ...
            children: [
              ...
              GestureDetector(
                onPanDown: (_) {
                  _isDraggingFeatured = true;
                },
                onPanEnd: (_) {
                  _isDraggingFeatured = false;
                },
                onPanCancel: () {
                  _isDraggingFeatured = false;
                },
                child: PageView.builder(
                  ...
                  onPageChanged: _onFeaturedChanged,
                  physics: const BouncingScrollPhysics(),
                  itemBuilder: (context, index) {
                    final image = trending[index].toString();
                ...
                child: Column(
                  ...
                  children: [
                    AnimatedSwitcher(
                      ...
                      child: Text(
                        getWallpaperName(
                          trending[_featuredIndex % trending.length].toString(),
                        ),
                        key: ValueKey(
                          trending[_featuredIndex % trending.length],
                        ),
                        maxLines: 2,
              ...
              Positioned.fill(
                child: Material(
                  ...
                  child: InkWell(
                    borderRadius: BorderRadius.circular(31.r),
                    onTap: () {
                      final image = trending[_featuredIndex % trending.length]
                          .toString();

                      _openPreview(image, category: 'Featured');
                    },
                  ),
                ),
              ),
...
// ============================================================
  ...
  Widget _buildExploreHeader() {
    return Padding(
      padding: EdgeInsets.fromLTRB(19.w, 32.h, 19.w, 14.h),
      child: Row(
        crossAxisAlignment: CrossAxisAlignment.end,
        children: [
          Expanded(
...
// ============================================================
  ...
  Widget _buildCategories(Map<String, dynamic> categories) {
    ...
    return SizedBox(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final wallpapers = List<dynamic>.from(
            category['wallpapers'] ?? const [],
          );
...
// ============================================================
  ...
  Widget _buildTrendingHeader() {
    return Padding(
      padding: EdgeInsets.fromLTRB(19.w, 35.h, 19.w, 13.h),
      child: Row(
        children: [
          Expanded(
            child: Column(
...
// ============================================================
  ...
  double _feedCardHeight(int index) {
    final heights = [265.h, 205.h, 290.h, 230.h, 275.h, 215.h];
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                border: Border.all(color: _accent.withOpacity(.12)),
              ),
              child: Icon(Icons.wifi_off_rounded, color: _accent, size: 32.sp),
            ),
            ...
            Text(
              ...
              textAlign: TextAlign.center,
              style: GoogleFonts.roboto(color: _secondary, fontSize: 13.sp),
            ),
...
// ============================================================
  ...
  String getWallpaperName(String url) {
    ...
    fileName = fileName.replaceAll(RegExp(r'-\d+x\d+-\d+$'), '');
...
class _FeaturedImageState extends State<_FeaturedImage>
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: Listenable.merge([widget.controller, _liveController]),
      builder: (context, child) {
        ...
        final pageOffset = widget.index - page;
        final distance = pageOffset.abs().clamp(0.0, 1.5);
        final t = _liveController.value * math.pi * 2;

// Slow, continuous "alive" movement while the page is resting.
        final liveX = math.sin(t) * 4.0;
        final liveY = math.cos(t * 0.72) * 2.5;
        final liveScale = 1.038 + math.sin(t * 0.82) * 0.008;

// Extra cinematic movement while swiping between pages.
        final swipeX = pageOffset * 18.w;
        final swipeRotate = pageOffset.clamp(-1.0, 1.0) * 0.006;
        final swipeScale = 1.0 - distance * .012;

        return ClipRect(
          child: Transform.translate(
            offset: Offset(liveX + swipeX, liveY),
            child: Transform.rotate(
              angle: swipeRotate,
              alignment: Alignment.center,
              child: Transform.scale(
                scale: liveScale * swipeScale,
                alignment: Alignment.center,
                child: child,
              ),
            ),
...
class _FeaturedIndicator extends StatelessWidget {
  final int count;
  final int current;

  const _FeaturedIndicator({required this.count, required this.current});

  @override
  Widget build(BuildContext context) {
    final visible = count > 6 ? 6 : count;
    ...
    return Row(
      children: List.generate(visible, (index) {
        ...
        return AnimatedContainer(
          ...
          height: 4.h,
          decoration: BoxDecoration(
            color: active ? Colors.white : Colors.white.withOpacity(.30),
            borderRadius: BorderRadius.circular(50.r),
          ),
        );
      }),
...
class _FeedWallpaperCard extends StatelessWidget {
  final String image;
  final String title;
  final int index;
  final double height;
  ...
  @override
  Widget build(BuildContext context) {
    final expandedHeight = height + 145.h;
